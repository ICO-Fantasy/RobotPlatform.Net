...
public sealed class ViewportController : BaseObject, IDisposable
{
    ...
    public IntPtr InitWindow(IntPtr parentHWnd, Int32Rect initialRect)
    {
        ...
        style =
            Win32Api.WS_VISIBLE
            | (parentHWnd == IntPtr.Zero ? Win32Api.WS_POPUP : Win32Api.WS_CHILD);
    ...
    void _UpdateParameter()
    {
        var parameterSet = InteractiveContext.Current.Parameters.Get<ViewportParameterSet>();
        _SetViewCube(
            parameterSet.ShowViewCube,
            parameterSet.ViewCubeSize,
            parameterSet.ViewCubeAnimationDuration
        );
    ...
    )
    {
        ...
        switch (_CurrentMouseMoveMode)
        {
            case MouseMoveMode.Panning:
                Viewport.V3dView.Pan(
                    (int)(pos.X - _LastMousePosition.X),
                    -(int)(pos.Y - _LastMousePosition.Y),
                    1.0,
                    true
                );
            ...
            case MouseMoveMode.Rotating:
                // Turntable
                Rotate(
                    (_LastMousePosition.X - pos.X) / 6.0,
                    (_LastMousePosition.Y - pos.Y) / 6.0,
                    0
                );
                break;

            case MouseMoveMode.Zooming:
                Viewport.V3dView.ZoomAtPoint(
    ...
    void _SetMouseMoveMode(MouseMoveMode mode)
    {
        switch (mode)
        {
            ...
            case MouseMoveMode.Zooming:
                Viewport.V3dView.StartZoomAtPoint(
                    (int)(_StartedMousePosition.X),
                    (int)(_StartedMousePosition.Y)
                );
    ...
    public void Rotate(double yawDeg, double pitchDeg, double rollDeg)
    {
        if (!_LockedToPlane)
        {
            if (Math.Abs(yawDeg) > 0.001 || Math.Abs(pitchDeg) > 0.001)
            {
                ...
                var angleLeft =
                    _OrbitProjectionConstraint - Ax2.XOY.Angle(new Ax2(Pnt.Origin, upDir));
    ...
    void _SetViewCube(bool isVisible, uint size, double duration)
    {
        ...
        _AisViewCube.SetTransformPersistence(
            new Graphic3d_TransformPers(
                Graphic3d_TransModeFlags.TriedronPers,
                Aspect_TypeOfTriedronPosition.RIGHT_UPPER,
                new Graphic3d_Vec2i(100, 100)
            )
        );
        ...
        _AisViewCube
            ...
            .ShadingAspect()
            .SetColor(Colors.Highlight.ToQuantityColor());
        ...
        if (isVisible)
        {
            ...
            {
                aisContext.SetViewAffinity(
                    _AisViewCube,
                    viewport.V3dView,
                    ReferenceEquals(viewport, Viewport)
                );
            }
        }
    ...
    int[] _CalcRectangleSelectionPoints(bool bottomUp)
    {
        ...
        int right = Math.Min(
            width,
            Math.Max((int)_StartedMousePosition.X, (int)_LastMousePosition.X)
        );
        int top = Math.Max(0, Math.Min((int)_StartedMousePosition.Y, (int)_LastMousePosition.Y));
        int bottom = Math.Min(
            height,
            Math.Max((int)_StartedMousePosition.Y, (int)_LastMousePosition.Y)
        );
    ...
    void _UpdateRubberbandSelection()
    {
        switch (_RubberbandMode)
        {
            ...
            case RubberbandSelectionMode.Freehand:
                ...
                if (distX * distX + distY * distY > RUBBERBAND_FREEHAND_SELECTION_THRESHOLD_SQUARED)
                {
    ...
    public void StartRubberbandSelection(
    ...
    void _StopRubberbandSelection()
    {
        if (_AisRubberBand != null)
        {
            ...
            {
                case RubberbandSelectionMode.Rectangle:
                    WorkspaceController.SelectByRectangle(
                        _CalcRectangleSelectionPoints(false),
                        _RubberbandIncludeTouched,
                        this
                    );
                    break;
                case RubberbandSelectionMode.Freehand:
                    ...
                    WorkspaceController.SelectByPolyline(
                        _RubberbandPoints,
                        _RubberbandIncludeTouched,
                        this
                    );
