...
public ref class BRepGProp sealed
    : public RDC::OCC::BaseClass<::BRepGProp>
{
...
public:
    BRepGProp(::BRepGProp* nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp>( nativeInstance, true )
    {}

    BRepGProp(::BRepGProp& nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp>( &nativeInstance, false )
    {}
...
public:
    ...
    /// </summary>
    static void LinearProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ LProps, bool SkipShared, bool UseTriangulation);
    /// <summary>
    /// Computes the linear global properties of the shape S,
    /// i.e. the global properties induced by each edge of the
    /// shape S, and brings them together with the global
    ...
    /// triangulation are used first.
    /// </summary>
    static void LinearProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ LProps, bool SkipShared);
    /// <summary>
    /// Computes the linear global properties of the shape S,
    /// i.e. the global properties induced by each edge of the
    /// shape S, and brings them together with the global
    ...
    /// triangulation are used first.
    /// </summary>
    static void LinearProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ LProps);
    /// <summary>
    ...
    /// </summary>
    static void SurfaceProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ SProps, bool SkipShared, bool UseTriangulation);
    /// <summary>
    /// Computes the surface global properties of the
    /// shape S, i.e. the global properties induced by each
    /// face of the shape S, and brings them together with
    ...
    /// otherwise face triangulations are used first.
    /// </summary>
    static void SurfaceProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ SProps, bool SkipShared);
    /// <summary>
    /// Computes the surface global properties of the
    /// shape S, i.e. the global properties induced by each
    /// face of the shape S, and brings them together with
    ...
    /// otherwise face triangulations are used first.
    /// </summary>
    static void SurfaceProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ SProps);
    /// <summary>
    ...
    /// are taken into calculation only once.
    /// </summary>
    static double SurfaceProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ SProps, double Eps, bool SkipShared);
    /// <summary>
    /// Updates <SProps> with the shape <S>, that contains its principal properties.
    /// The surface properties of all the faces in <S> are computed.
    /// Adaptive 2D Gauss integration is used.
    ...
    /// are taken into calculation only once.
    /// </summary>
    static double SurfaceProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ SProps, double Eps);
    /// <summary>
    /// 
    ...
    /// otherwise face triangulations are used first.
    /// </summary>
    static void VolumeProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, bool OnlyClosed, bool SkipShared, bool UseTriangulation);
    /// <summary>
    /// 
    /// Computes the global volume properties of the solid
    /// S, and brings them together with the global
    ...
    /// otherwise face triangulations are used first.
    /// </summary>
    static void VolumeProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, bool OnlyClosed, bool SkipShared);
    /// <summary>
    /// 
    /// Computes the global volume properties of the solid
    /// S, and brings them together with the global
    ...
    /// otherwise face triangulations are used first.
    /// </summary>
    static void VolumeProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, bool OnlyClosed);
    /// <summary>
    /// 
    /// Computes the global volume properties of the solid
    /// S, and brings them together with the global
    ...
    /// otherwise face triangulations are used first.
    /// </summary>
    static void VolumeProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps);
    /// <summary>
    ...
    /// </summary>
    static double VolumeProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps, bool OnlyClosed, bool SkipShared);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// faces are taken into calculation only once.
    /// </summary>
    static double VolumeProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps, bool OnlyClosed);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// faces are taken into calculation only once.
    /// </summary>
    static double VolumeProperties(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// </summary>
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps, bool OnlyClosed, bool IsUseSpan, bool CGFlag, bool IFlag, bool SkipShared);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// (the same TShape, location and orientation) faces are taken into calculation only once.
    /// </summary>
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps, bool OnlyClosed, bool IsUseSpan, bool CGFlag, bool IFlag);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// (the same TShape, location and orientation) faces are taken into calculation only once.
    /// </summary>
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps, bool OnlyClosed, bool IsUseSpan, bool CGFlag);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// (the same TShape, location and orientation) faces are taken into calculation only once.
    /// </summary>
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps, bool OnlyClosed, bool IsUseSpan);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// (the same TShape, location and orientation) faces are taken into calculation only once.
    /// </summary>
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps, bool OnlyClosed);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// (the same TShape, location and orientation) faces are taken into calculation only once.
    /// </summary>
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, double Eps);
    /// <summary>
    /// Updates <VProps> with the shape <S>, that contains its principal properties.
    /// The volume properties of all the FORWARD and REVERSED faces in <S> are computed.
    /// If OnlyClosed is True then computed faces must belong to closed Shells.
    ...
    /// (the same TShape, location and orientation) faces are taken into calculation only once.
    /// </summary>
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps);
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, RDC::OCC::Pln thePln, double Eps, bool OnlyClosed, bool IsUseSpan, bool CGFlag, bool IFlag, bool SkipShared);
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, RDC::OCC::Pln thePln, double Eps, bool OnlyClosed, bool IsUseSpan, bool CGFlag, bool IFlag);
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, RDC::OCC::Pln thePln, double Eps, bool OnlyClosed, bool IsUseSpan, bool CGFlag);
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, RDC::OCC::Pln thePln, double Eps, bool OnlyClosed, bool IsUseSpan);
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, RDC::OCC::Pln thePln, double Eps, bool OnlyClosed);
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, RDC::OCC::Pln thePln, double Eps);
    static double VolumePropertiesGK(RDC::OCC::TopoDS_Shape^ S, RDC::OCC::GProp_GProps^ VProps, RDC::OCC::Pln thePln);
}; // class BRepGProp
...
public:
    BRepGProp_Cinert(::BRepGProp_Cinert* nativeInstance)
        : RDC::OCC::GProp_GProps( nativeInstance )
    {}
...
public:
    BRepGProp_Cinert();
    BRepGProp_Cinert(RDC::OCC::BRepAdaptor_Curve^ C, RDC::OCC::Pnt CLocation);
    void SetLocation(RDC::OCC::Pnt CLocation);
    void Perform(RDC::OCC::BRepAdaptor_Curve^ C);
}; // class BRepGProp_Cinert
...
public ref class BRepGProp_Domain sealed
    : public RDC::OCC::BaseClass<::BRepGProp_Domain>
{
...
public:
    BRepGProp_Domain(::BRepGProp_Domain* nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp_Domain>( nativeInstance, true )
    {}

    BRepGProp_Domain(::BRepGProp_Domain& nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp_Domain>( &nativeInstance, false )
    {}
...
public:
    ...
    /// </summary>
    BRepGProp_Domain(RDC::OCC::TopoDS_Face^ F);
    /// <summary>
...
public ref class BRepGProp_EdgeTool sealed
    : public RDC::OCC::BaseClass<::BRepGProp_EdgeTool>
{
...
public:
    BRepGProp_EdgeTool(::BRepGProp_EdgeTool* nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp_EdgeTool>( nativeInstance, true )
    {}

    BRepGProp_EdgeTool(::BRepGProp_EdgeTool& nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp_EdgeTool>( &nativeInstance, false )
    {}
...
public:
    ...
    /// </summary>
    static int IntegrationOrder(RDC::OCC::BRepAdaptor_Curve^ C);
    /// <summary>
    /// Returns the point of parameter U on the loaded curve.
    /// </summary>
    static RDC::OCC::Pnt Value(RDC::OCC::BRepAdaptor_Curve^ C, double U);
    /// <summary>
    ...
    /// </summary>
    static int NbIntervals(RDC::OCC::BRepAdaptor_Curve^ C, RDC::OCC::GeomAbs_Shape S);
    /// <summary>
    ...
    /// </summary>
    static void Intervals(RDC::OCC::BRepAdaptor_Curve^ C, RDC::OCC::TColStd_Array1OfReal^ T, RDC::OCC::GeomAbs_Shape S);
}; // class BRepGProp_EdgeTool
...
public ref class BRepGProp_Face sealed
    : public RDC::OCC::BaseClass<::BRepGProp_Face>
{
...
public:
    BRepGProp_Face(::BRepGProp_Face* nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp_Face>( nativeInstance, true )
    {}

    BRepGProp_Face(::BRepGProp_Face& nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp_Face>( &nativeInstance, false )
    {}
...
public:
    ...
    /// and GetTKnots.
    /// </summary>
    BRepGProp_Face(RDC::OCC::TopoDS_Face^ F, bool IsUseSpan);
    /// <summary>
    /// Constructor. Initializes the object with the face and the
    /// flag IsUseSpan that says if it is necessary to define
    /// spans on a face. This option has an effect only for
    ...
    /// and GetTKnots.
    /// </summary>
    BRepGProp_Face(RDC::OCC::TopoDS_Face^ F);
    void Load(RDC::OCC::TopoDS_Face^ F);
    ...
    /// </summary>
    RDC::OCC::TopoDS_Face^ GetFace();
    /// <summary>
    /// Returns the value of the boundary curve of the face.
    /// </summary>
    RDC::OCC::Pnt2d Value2d(double U);
    int SIntOrder(double Eps);
    int SVIntSubs();
    int SUIntSubs();
    void UKnots(RDC::OCC::TColStd_Array1OfReal^ Knots);
    void VKnots(RDC::OCC::TColStd_Array1OfReal^ Knots);
    int LIntOrder(double Eps);
    int LIntSubs();
    void LKnots(RDC::OCC::TColStd_Array1OfReal^ Knots);
    /// <summary>
    ...
    /// </summary>
    void Normal(double U, double V, RDC::OCC::Pnt% P, RDC::OCC::Vec% VNor);
    /// <summary>
    /// Loading the boundary arc.
    /// Returns FALSE if edge has no P-Curve.
    /// </summary>
    bool Load(RDC::OCC::TopoDS_Edge^ E);
    /// <summary>
    /// Returns the parametric value of the start point of
    ...
    /// </summary>
    void D12d(double U, RDC::OCC::Pnt2d% P, RDC::OCC::Vec2d% V1);
    /// <summary>
    ...
    /// </summary>
    void Load(bool IsFirstParam, RDC::OCC::GeomAbs_IsoType theIsoType);
    /// <summary>
    ...
    /// </summary>
    void GetUKnots(double theUMin, double theUMax, RDC::OCC::TColStd_HArray1OfReal^ theUKnots);
    /// <summary>
    ...
    /// </summary>
    void GetTKnots(double theTMin, double theTMax, RDC::OCC::TColStd_HArray1OfReal^ theTKnots);
}; // class BRepGProp_Face
...
public ref class BRepGProp_Gauss sealed
    : public RDC::OCC::BaseClass<::BRepGProp_Gauss>
{
...
public:
    BRepGProp_Gauss(::BRepGProp_Gauss* nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp_Gauss>( nativeInstance, true )
    {}

    BRepGProp_Gauss(::BRepGProp_Gauss& nativeInstance)
        : RDC::OCC::BaseClass<::BRepGProp_Gauss>( &nativeInstance, false )
    {}
...
public:
    ...
    /// Constructor
    /// </summary>
    BRepGProp_Gauss(RDC::OCC::BRepGProp_Gauss::BRepGProp_GaussType theType);
    /// <summary>
    ...
    /// - matrix of inertia;
    /// </param>
    void Compute(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt theLocation, double theCoeff, bool theIsByPoint, double% theOutMass, RDC::OCC::Pnt% theOutGravityCenter, RDC::OCC::Mat% theOutInertia);
    /// <summary>
    ...
    /// - matrix of inertia;
    /// </param>
    void Compute(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt theLocation, double% theOutMass, RDC::OCC::Pnt% theOutGravityCenter, RDC::OCC::Mat% theOutInertia);
    /// <summary>
    ...
    /// - matrix of inertia;
    /// </param>
    void Compute(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt theLocation, double theCoeff, bool theIsByPoint, double% theOutMass, RDC::OCC::Pnt% theOutGravityCenter, RDC::OCC::Mat% theOutInertia);
    /// <summary>
    /// Computes the global properties of a surface. Surface can be closed.
    /// The method is quick and its precision is enough for many cases of analytical surfaces.
    /// Non-adaptive 2D Gauss integration with predefined numbers of Gauss points
    ...
    /// - matrix of inertia;
    /// </param>
    void Compute(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt theLocation, double% theOutMass, RDC::OCC::Pnt% theOutGravityCenter, RDC::OCC::Mat% theOutInertia);
    /// <summary>
    ...
    /// for two successive steps of adaptive integration.
    /// </returns>
    double Compute(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt theLocation, double theEps, double theCoeff, bool theByPoint, double% theOutMass, RDC::OCC::Pnt% theOutGravityCenter, RDC::OCC::Mat% theOutInertia);
    /// <summary>
    ...
    /// for two successive steps of adaptive integration.
    /// </returns>
    double Compute(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt theLocation, double theEps, double% theOutMass, RDC::OCC::Pnt% theOutGravityCenter, RDC::OCC::Mat% theOutInertia);
}; // class BRepGProp_Gauss
...
public:
    ...
    /// of polylines  represented by set of points.
    /// </summary>
    void Perform(RDC::OCC::TColgp_Array1OfPnt^ theNodes);
    /// <summary>
    ...
    /// </summary>
    static void PreparePolygon(RDC::OCC::TopoDS_Edge^ theE, RDC::OCC::TColgp_HArray1OfPnt^ thePolyg);
}; // class BRepGProp_MeshCinert
...
public:
    ...
    /// </summary>
    BRepGProp_MeshProps(RDC::OCC::BRepGProp_MeshProps::BRepGProp_MeshObjType theType);
    /// <summary>
    /// Sets the point relative which the calculation is to be done
    /// </summary>
    void SetLocation(RDC::OCC::Pnt theLocation);
    ...
    /// </summary>
    static void CalculateProps(RDC::OCC::Pnt p1, RDC::OCC::Pnt p2, RDC::OCC::Pnt p3, RDC::OCC::Pnt Apex, bool isVolume, double% GProps, int NbGaussPoints, double GaussPnts);
    /// <summary>
    /// Get type of mesh object
    /// </summary>
    RDC::OCC::BRepGProp_MeshProps::BRepGProp_MeshObjType GetMeshObjType();
}; // class BRepGProp_MeshProps
...
public:
    BRepGProp_Sinert();
    BRepGProp_Sinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pnt SLocation);
    /// <summary>
    ...
    /// </summary>
    BRepGProp_Sinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pnt SLocation);
    BRepGProp_Sinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pnt SLocation, double Eps);
    BRepGProp_Sinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pnt SLocation, double Eps);
    void SetLocation(RDC::OCC::Pnt SLocation);
    void Perform(RDC::OCC::BRepGProp_Face^ S);
    void Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D);
    double Perform(RDC::OCC::BRepGProp_Face^ S, double Eps);
    double Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, double Eps);
    /// <summary>
...
public:
    ...
    /// Error of the computation is not calculated.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pnt VLocation);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface <S> and the point VLocation. S can be closed
    /// Adaptive 2D Gauss integration is used.
    ...
    /// for two successive steps of adaptive integration.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pnt VLocation, double Eps);
    /// <summary>
    ...
    /// Error of the computation is not calculated.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pnt O, RDC::OCC::Pnt VLocation);
    /// <summary>
    /// Computes the global properties of the region of 3D space
    /// delimited with the surface <S> and the point VLocation.
    /// Adaptive 2D Gauss integration is used.
    ...
    /// WARNING: if Eps > 0.001 algorithm performs non-adaptive integration.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pnt O, RDC::OCC::Pnt VLocation, double Eps);
    /// <summary>
    /// Computes the global properties of the region of 3D space
    ...
    /// Error of the computation is not calculated.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pln Pl, RDC::OCC::Pnt VLocation);
    /// <summary>
    /// Computes the global properties of the region of 3D space
    /// delimited with the surface <S> and the plane Pln.
    /// Adaptive 2D Gauss integration is used.
    ...
    /// WARNING: if Eps > 0.001 algorithm performs non-adaptive integration.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pln Pl, RDC::OCC::Pnt VLocation, double Eps);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface <S> and the point VLocation. S can be closed
    /// The method is quick and its precision is enough for many cases of analytical
    ...
    /// Error of the computation is not calculated.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pnt VLocation);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface <S> and the point VLocation. S can be closed
    /// Adaptive 2D Gauss integration is used.
    ...
    /// for two successive steps of adaptive integration.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pnt VLocation, double Eps);
    /// <summary>
    /// Computes the global properties of the region of 3D space
    /// delimited with the surface <S> and the point VLocation.
    /// The method is quick and its precision is enough for many cases of analytical
    ...
    /// Error of the computation is not calculated.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pnt O, RDC::OCC::Pnt VLocation);
    /// <summary>
    /// Computes the global properties of the region of 3D space
    /// delimited with the surface <S> and the point VLocation.
    /// Adaptive 2D Gauss integration is used.
    ...
    /// WARNING: if Eps > 0.001 algorithm performs non-adaptive integration.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pnt O, RDC::OCC::Pnt VLocation, double Eps);
    /// <summary>
    /// Computes the global properties of the region of 3D space
    /// delimited with the surface <S> and the plane Pln.
    /// The method is quick and its precision is enough for many cases of analytical
    ...
    /// Error of the computation is not calculated.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pln Pl, RDC::OCC::Pnt VLocation);
    /// <summary>
    /// Computes the global properties of the region of 3D space
    /// delimited with the surface <S> and the plane Pln.
    /// Adaptive 2D Gauss integration is used.
    ...
    /// WARNING: if Eps > 0.001 algorithm performs non-adaptive integration.
    /// </summary>
    BRepGProp_Vinert(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pln Pl, RDC::OCC::Pnt VLocation, double Eps);
    void SetLocation(RDC::OCC::Pnt VLocation);
    void Perform(RDC::OCC::BRepGProp_Face^ S);
    double Perform(RDC::OCC::BRepGProp_Face^ S, double Eps);
    void Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pnt O);
    double Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pnt O, double Eps);
    void Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pln Pl);
    double Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::Pln Pl, double Eps);
    void Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D);
    double Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, double Eps);
    void Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pnt O);
    double Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pnt O, double Eps);
    void Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pln Pl);
    double Perform(RDC::OCC::BRepGProp_Face^ S, RDC::OCC::BRepGProp_Domain^ D, RDC::OCC::Pln Pl, double Eps);
    /// <summary>
...
public:
    ...
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the point VLocation.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the point VLocation.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt theLocation, double theTolerance);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the point VLocation.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt theLocation);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    ...
    /// respect to thePoint.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt thePoint, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the point VLocation. The inertia is computed with
    /// respect to thePoint.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt thePoint, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the point VLocation. The inertia is computed with
    /// respect to thePoint.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt thePoint, RDC::OCC::Pnt theLocation, double theTolerance);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the point VLocation. The inertia is computed with
    /// respect to thePoint.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt thePoint, RDC::OCC::Pnt theLocation);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the point VLocation.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the point VLocation.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the point VLocation.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt theLocation, double theTolerance);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the point VLocation.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt theLocation);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the point VLocation. The inertia is computed with
    /// respect to thePoint.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt thePoint, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the point VLocation. The inertia is computed with
    /// respect to thePoint.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt thePoint, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the point VLocation. The inertia is computed with
    /// respect to thePoint.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt thePoint, RDC::OCC::Pnt theLocation, double theTolerance);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the point VLocation. The inertia is computed with
    /// respect to thePoint.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt thePoint, RDC::OCC::Pnt theLocation);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the plane.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pln thePlane, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the plane.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pln thePlane, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the plane.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pln thePlane, RDC::OCC::Pnt theLocation, double theTolerance);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the naturally restricted surface
    /// and the plane.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pln thePlane, RDC::OCC::Pnt theLocation);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the plane.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pln thePlane, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the plane.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pln thePlane, RDC::OCC::Pnt theLocation, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the plane.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pln thePlane, RDC::OCC::Pnt theLocation, double theTolerance);
    /// <summary>
    /// Constructor. Computes the global properties of a region of
    /// 3D space delimited with the surface bounded by the domain
    /// and the plane.
    /// </summary>
    BRepGProp_VinertGK(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pln thePlane, RDC::OCC::Pnt theLocation);
    /// <summary>
    ...
    /// point VLocation.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// point VLocation.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// point VLocation.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, double theTolerance);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// point VLocation.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    ...
    /// thePoint.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt thePoint, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// point VLocation. The inertia is computed with respect to
    /// thePoint.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt thePoint, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// point VLocation. The inertia is computed with respect to
    /// thePoint.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt thePoint, double theTolerance);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// point VLocation. The inertia is computed with respect to
    /// thePoint.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pnt thePoint);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// point VLocation.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// point VLocation.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// point VLocation.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, double theTolerance);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// point VLocation.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// point VLocation. The inertia is computed with respect to
    /// thePoint.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt thePoint, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// point VLocation. The inertia is computed with respect to
    /// thePoint.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt thePoint, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// point VLocation. The inertia is computed with respect to
    /// thePoint.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt thePoint, double theTolerance);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// point VLocation. The inertia is computed with respect to
    /// thePoint.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pnt thePoint);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// plane.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pln thePlane, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// plane.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pln thePlane, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// plane.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pln thePlane, double theTolerance);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the naturally restricted surface and the
    /// plane.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::Pln thePlane);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// plane.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pln thePlane, double theTolerance, bool theCGFlag, bool theIFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// plane.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pln thePlane, double theTolerance, bool theCGFlag);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// plane.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pln thePlane, double theTolerance);
    /// <summary>
    /// Computes the global properties of a region of 3D space
    /// delimited with the surface bounded by the domain and the
    /// plane.
    /// </summary>
    double Perform(RDC::OCC::BRepGProp_Face^ theSurface, RDC::OCC::BRepGProp_Domain^ theDomain, RDC::OCC::Pln thePlane);
    /// <summary>
